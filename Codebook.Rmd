...
## Summary
This CodeBook describes the variables, the data, and any transformations or work done to clean up a dataset - [Human Activity Recognition Using Smartphones](https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip ) - following the 5 requests provided by the **Getting and Cleaning Data Course Project Instructions**.
...
* `activity_ID` is a unique ID for each activity_Label
* all other features above are fully explained in `features_info.txt` in the [original dataset](https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip). In short, these are coming from the accelerometer and gyroscope 3-axial raw signals tAcc-XYZ and tGyro-XYZ.
...
The following steps are taken by `run_analysis.R` to generate `finalTidyDataSet.csv` from the original dataset :
...
2. Quick sanity checks are performed :
    ...
    + Another quick sanity check is done in order to make sure that names that will be used for new columns (when merging data) were not used before.
3. Names to the dataframes columns are attributed according documentation :
    ...
    + **This step addresses, in advance, project request item #4**
4. Train data is merged with column bind. The same is done by test data. Then, a row bind is performed with the two datasets generating a data frame `all_data` containing all data from `X_train.txt`, `X_test.txt`, `y_train.txt`, `y_test.txt`, `subject_train.txt`, `subject_test.txt`.
    + **This step addresses project request item #1**
5. A new dataset `filtered_data` is create with `all_data` features that have `mean(` or `std(` (mean and standard deviation) and the columns `activity_ID`, `subject_ID`.
    + **This step addresses project request item #2**
6. A new column `activity_Label` is added to the dataframe `filtered_data` containing the label for the respective `activity_ID` according `activity_labels.txt` (loaded in `activity_Labels` dataframe)
    + **This step addresses project request item #3**
7. `final_dataset` is generated by aggregating data frame `filtered_data` with the average of each variable for each activity and each subject.
    ...
    + **This step addresses project request item #5**
